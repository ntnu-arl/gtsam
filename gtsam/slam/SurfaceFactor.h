...
namespace gtsam
{
  ...
  class GTSAM_EXPORT SurfaceFactor3JL
      : public gtsam::NoiseModelFactor3<gtsam::Pose3, gtsam::Pose3, gtsam::Pose3>
  {
    gtsam::Point3 j0_; // In the frame of the first key
    gtsam::Point3 l0_; // In the frame of the first key
    gtsam::Point3 m1_; // In the frame of the second key
    gtsam::Point3 i2_; // In the frame of the third key

  public:
    typedef gtsam::NoiseModelFactor3<gtsam::Pose3, gtsam::Pose3, gtsam::Pose3> Base;
    typedef SurfaceFactor3JL This;
    typedef boost::shared_ptr<This> shared_ptr;

    SurfaceFactor3JL(
        const gtsam::Point3 &j0, gtsam::Key k0, const gtsam::Point3 &l0,
        const gtsam::Point3 &m1, gtsam::Key k1, const gtsam::Point3 &i2, gtsam::Key k2,
        const gtsam::SharedNoiseModel &model)
        : Base(model, k0, k1, k2),
          j0_(j0),
          l0_(l0),
        ...
        boost::optional<gtsam::Matrix &> H2 = boost::none) const override
    {
      ...
      {
        gtsam::Matrix36 d_n_w_hat = (gtsam::Matrix33::Identity() - n_w_hat * n_w_hat.transpose()) *
                                    (gtsam::skewSymmetric(l_w - m_w) -
                                     gtsam::skewSymmetric(j_w - m_w)) *
                                    (-Hm) /
                                    n_w.norm();
